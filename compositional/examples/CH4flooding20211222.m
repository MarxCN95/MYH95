...
for step = 1:numel(statespc)
    for i = 1:ncomp
    compliquidM4(step,i) = statespc{step,1}.x(138,i);
    end
end
plot(dt,compliquidM4);
legend(eos.fluid.names);
%% %双重效应的绘制states.y，基质中各组分的气相百分比。
figure;
compvaporM4 = [ ] ;
for step = 1:numel(statespc)
    for i = 1:ncomp
    compvaporM4(step,i) = statespc{step,1}.y(138,i);
    end
end
plot(dt,compvaporM4);
...
for step = 1:numel(states)
    for i = 1:ncomp
    compliquidM2(step,i) = states{step,1}.x(138,i);
    end
end
plot(dt,compliquidM2);
legend(eos.fluid.names);
%% %仅临界性质偏移的绘制states.y，基质中各组分的气相百分比。
figure;
compvaporM2 = [ ] ;
for step = 1:numel(states)
    for i = 1:ncomp
    compvaporM2(step,i) = states{step,1}.y(138,i);
    end
end
plot(dt,compvaporM2);
legend('气相', eos.fluid.names);