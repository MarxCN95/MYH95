...
v = [0, 90];
figure('name','不考虑毛管力','Position',[300, 300, 600, 500]); %clf;
...
%%
v = [0, 90];
figure('name','考虑毛管力','Position',[300, 300, 600, 500]); %clf;
...
for step = 1:numel(statespc)
    for i = 1:ncomp
    compliquidM3(step,i) = statespc{step,1}.x(138,i);
    end
end
plot(dt,compliquidM3);
legend(eos.fluid.names);
%% %仅毛管力的绘制states.y，基质中各组分的气相百分比。
figure;
compvaporM3 = [ ] ;
for step = 1:numel(statespc)
    for i = 1:ncomp
    compvaporM3(step,i) = statespc{step,1}.y(138,i);
    end
end
plot(dt,compvaporM3);
...
for step = 1:numel(states)
    for i = 1:ncomp
    compliquidM1(step,i) = states{step,1}.x(138,i);
    end
end
plot(dt,compliquidM1);
legend(eos.fluid.names);
%% %基础方案的绘制states.y，基质中各组分的气相百分比。
figure;
compvaporM1 = [ ] ;
for step = 1:numel(states)
    for i = 1:ncomp
    compvaporM1(step,i) = states{step,1}.y(138,i);
    end
end
plot(dt,compvaporM1);
legend('气相', eos.fluid.names);
...
SO3 = [ ] ;
for step = 1:numel(statespc)
    SO3(step,1) = statespc{step,1}.s(138,2);
end
plot(dt,SO3);
legend('SO');
%% 基础方案的SO
...
SO1 = [ ] ;
for step = 1:numel(states)
    SO1(step,1) = states{step,1}.s(138,2);
end
plot(dt,SO1);
legend('SO');
%% 仅毛管力的SG
...
SG3 = [ ] ;
for step = 1:numel(statespc)
    SG3(step,1) = statespc{step,1}.s(138,3);
end
plot(dt,SG3);
legend('SO');
%% 基础方案的SO
...
SG1 = [ ] ;
for step = 1:numel(states)
    SG1(step,1) = states{step,1}.s(138,3);
end
plot(dt,SG1);
legend('SO');