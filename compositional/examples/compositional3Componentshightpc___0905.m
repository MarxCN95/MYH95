...
v = [0, 90];
figure('name','考虑毛管力','Position',[100, 400, 1300, 500]); %clf;
...
statespressure = [ ];
for step = 1:numel(states)
    statespressure(step,1) = states{step,1}.pressure(2013,1);
end
...
%% plot figure
v = [0, 90];
figure('name','不考虑毛管力','Position',[600, 400, 1300, 500]); %clf;
...
statespressurenon = [ ];
for step = 1:numel(statesnon)
    statespressurenon(step,1) = statesnon{step,1}.pressure(2013,1);
end
...
statesoilrhonon = [ ] ;
for step = 1:numel(statesnon)
    statesoilrhonon(step,1) = statesnon{step,1}.rho(2013,1);
end
...
statesSO = [ ] ;
for step = 1:numel(states)
    statesSO(step,1) = states{step,1}.s(633,2);
end
plot(schstep,statesSO);
legend('SO');
%% %绘制states.s，裂缝中的含油饱和度。
figure;
statesSOF = [ ] ;
for step = 1:numel(states)
    statesSOF(step,1) = states{step,1}.s(622,2);
end
plot(schstep,statesSOF);
legend('SO');
%% %绘制statesnon.s，裂缝中的含油饱和度。
figure;
statesSOFnon = [ ] ;
for step = 1:numel(statesnon)
    statesSOFnon(step,1) = statesnon{step,1}.s(622,2);
end
plot(schstep,statesSOFnon);
legend('SO');
%% %绘制statesnon.s，基质中的含油饱和度。
figure;
statesSOnon = [ ] ;
for step = 1:numel(statesnon)
    statesSOnon(step,1) = statesnon{step,1}.s(633,2);
end
...
% figure;
% comparepressure = [ ] ;
% steptotal = 1:numel(statesnon);
% for step = 1:11
%     for i = 1:2
%         if i == 1 
%     comparepressure(step,i) = states{21,1}.pressure(step,1);
%         end
%         if i == 2 
%     comparepressure(step,i) = statesnon{21,1}.pressure(step,1);
%         end
%     end
% end
...
wsqOrnon = [ ] ;
for step = 1:numel(states)
    wsqOrnon(step,1) = wsnon{step,1}.qOr;
end
plot(schstep,wsqOrnon);
legend('QOR');
%% %绘制wsqGr，QGR。
figure;
x=1:numel(states);
wsqGrnon = [ ] ;
for step = 1:numel(states)
    wsqGrnon(step,1) = wsnon{step,1}.qGr;
end
plot(schstep,wsqGrnon);
legend('QGR');
%% %绘制states.components，基质中各组分的摩尔百分比。
figure;
x=1:numel(states);
...
for step = 1:numel(states)
    for i = 1:ncomp
    %statescomponentsM(step,i) = states{step,1}.components(633,i);
    %statescomponentsM(step,i) = states{step,1}.components(2013,i);
    statescomponentsM(step,i) = states{step,1}.components(622,i);
    end
end
plot(schstep,statescomponentsM);
legend(eos.fluid.names);
...
figure;
x=1:numel(states);
statescomponentsMnon = [ ] ;
for step = 1:numel(states)
    for i = 1:ncomp
    %statescomponentsMnon(step,i) = statesnon{step,1}.components(633,i);
        %statescomponentsMnon(step,i) = statesnon{step,1}.components(2013,i);
        statescomponentsMnon(step,i) = statesnon{step,1}.components(622,i);
    end
end
plot(schstep,statescomponentsMnon);
legend(eos.fluid.names);
%%
close all;
...
pcrecord = [];
for step = 1:numel(states)
pcog  = fluidint.pcOG(states{step,1}.s(:,2));
pcrecord(step ,1) = pcog(633,1);
end
pressurerecord = [];
for step = 1:numel(states)
pressuresingle = states{step,1}.pressure;
pressurerecord(step ,1) = pressuresingle(633,1);
end
GORDnon=qGs_Dnon./qOs_Dnon; GORD=qGs_D./qOs_D;