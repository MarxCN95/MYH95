classdef ThreePhaseCompositionalModel < ReservoirModel
    ...
    methods
        ...
        function scaling = getComponentScaling(model, state)
            ...
            scaling(wT  < 1e-3) = 1e6;%//计算平均密度，液体摩尔百分比*油密度800+气体摩尔百分比*气密度10
        end
        ...
        function [v_eqs, tolerances, names] = getConvergenceValues(model, problem, varargin)%//得到收敛误差
            ...
            % Define tolerances, values and names%//定义容差
            ...
            if any(strcmpi(problem.primaryVariables, 'pressure'))
                ...
                if isfinite(model.incTolPressure)
                    ...
                    if any(isp)
                       ...
                       %//以上注释为原语句，下面if语句为修改
                        if problem.iterationNo <= 20       %//修改，迭代6次后增大误差
                            tolerances = [model.incTolPressure, tolerances];
                        else
                            tolerances = [model.incTolPressure, tolerances]*10*(problem.iterationNo-20);                            
                        end
                    end
        ...
        function [compEqs, compSrc, eqNames, wellSol] = getExtraWellContributions(model, well, wellSol0, wellSol, q_s, bh, packed, qMass, qVol, dt, iteration)
            ...
            for cNo = 1:ncomp
                ...
                compSrc{N+cNo} = q_i;
                wellSol.components(:, cNo) = double(q_i);%//计算ws.components
            end
        end
...
%             end
    ...
    methods (Access=protected)
       function [v_comp, tol_comp, names_comp, isComponent] = getComponentConvergenceValues(model, problem)
            % Check convergence criterion of components%//检查组分的收敛标准
