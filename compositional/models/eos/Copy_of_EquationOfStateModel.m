...
%% %//以下部分复制添加了计算ZL
function Z = computeCompressibilityZL(model, p, xy, A, B, Si, Bi, isLiquid)
             if iscell(xy)
...
%% %//以上部分修改
        ...
        function state = validateState(model, state)%//验证状态
        ...
        function [Z_L, Z_V, f_L, f_V] = getProperties(model, P, T, x, y, z, sO, sG, state, varargin)
            %pccon = state.s(:, 1)*100;%//修改，添加计算毛管力
            if nargin < 9
                state = [];
            end
            [Si_L, Si_V, A_L, A_V, B_L, B_V, Bi] = model.getMixtureFugacityCoefficients(P, T, x, y, model.fluid.acentricFactors);
            if isfield(state, 'Z_L')
                ...
                Z_L = model.computeCompressibilityZL(P, x, A_L, B_L, Si_L, Bi, true);%//修改，计算压缩系数ZL
            end
            if isfield(state, 'Z_V')
                Z_V = state.Z_V;
            else
        ...
        %% %//以下为修改，复制计算FugacityfL
         function [f, phi] = computeFugacityfL(model, p, x, Z, A, B, Si, Bi)
            % Compute fugacity based on EOS coefficients
            [m1, m2] = model.getEOSCoefficients();
            ncomp = model.fluid.getNumberOfComponents();
            if iscell(x)
