classdef EquationOfStateModel < PhysicalModel
    ...
    methods
        function model = EquationOfStateModel(G, fluid, eosname)%//初始
            if nargin < 3
                eosname = 'PR';
            end
            ...
            model.fluid = fluid;
            model.nonlinearTolerance = 1e-4;%//非线性公差
        ...
        function Z = solveCubicEOS(model, A, B)
            ...
            % values.避免挑选任何复杂的根。至少有一个根是真实的。我们用NaN替换这些无效值，以便在使用min或max时，它们将忽略任何先前复杂的值。
            mtol = 0;
...
%% %//以下部分复制添加了计算Z_V
function Z = computeCompressibilityZ_V(model, p, xy, A_V, B_V, Si_V, Bi_V, isLiquid, state)
             if iscell(xy)
                xy = cellfun(@double, xy, 'UniformOutput', false);
                xy = [xy{:}];
            end
            if iscell(Si_V)
                Si_V = cellfun(@double, Si_V, 'UniformOutput', false);
                Si_V = [Si_V{:}];
            end
            if iscell(Bi_V)
                Bi_V = cellfun(@double, Bi_V, 'UniformOutput', false);
                Bi_V = [Bi_V{:}];
            end
            p = double(p); A_V = double(A_V); B_V = double(B_V);
            global pcswitch fluidint pcapillary;
             if pcswitch == 1
                 %pcog  = fluidint.pcOG(state.s(:,2));
%                  pcog =  pcapillary;
rho_L = model.PropertyModel.computeDensity(state.pressure, state.x, state.Z_L, state.T, 1);
rho_V = model.PropertyModel.computeDensity(state.pressure, state.y, state.Z_V, state.T, 0);
[Pcap] = computePcapfinal(model.fluid, rho_L,rho_V,state.x,state.y);
p = p + Pcap;
             end %//添加毛管力
            if ~model.selectGibbsMinimum
                if isLiquid
                    Z = computeLiquidZ(model, A_V, B_V);
                else
                    Z = computeVaporZ(model, A_V, B_V);
                end
                return
            end
            
            Z0 = model.solveCubicEOS(A_V, B_V);
            bad = bsxfun(@lt, Z0, B_V);
            Z0(bad) = nan;
            ...
            if any(multiple)
                ...
                [~, phi] = model.computeFugacity(p(multiple), xi, Z_max, A_V(multiple), B_V(multiple), Si_V(multiple, :), Bi_V(multiple, :));%//_V是否需要？
                g_max = sum(phi.*xi, 2);
                [~, phi] = model.computeFugacity(p(multiple), xi, Z_min, A_V(multiple), B_V(multiple), Si_V(multiple, :), Bi_V(multiple, :));%//_V是否需要？
                g_min = sum(phi.*xi, 2);
                Zi = Z_max;
...
%% %//以上部分修改
        ...
        function [state, report] = stepFunction(model, state, state0, dt, drivingForces, linsolve, nonlinsolve, iteration, varargin) %#ok
            ...
            % equation of state (thermodynamic flash calculation);计算给定状态方程的解决过程的单个步骤（热力学闪蒸计算）;
            T = state.T;
            P = state.pressure;
            nc = numel(P);
            assert(all(P >= 0))
            ...
            if iteration == 1
                ...
                [Si_L, Si_V, A_L, A_V, B_L, B_V, Bi, Bi_V] = model.getMixtureFugacityCoefficients(P, T, x0, y0, acf, state);%//修改
                % Solve EOS for each phase
                Z0_L = model.computeCompressibilityZ(state.pressure, x0, A_L, B_L, Si_L, Bi, true);
                Z0_V = model.computeCompressibilityZ_V(state.pressure, y0, A_V, B_V, Si_V, Bi_V, false, state);%//修改
            ...
            if any(active)
                ...
                if model.useNewton
                    % Newton-based solver for equilibrium牛顿求解器求平衡
                    [x, y, K, Z_L, Z_V, L, equilvals] = model.newtonCompositionUpdate(P, T, z, K, L);
                else
                    % Successive substitution solver for equilibrium连续替换求解器求平衡
                ...
                resConv = values <= model.nonlinearTolerance & iteration > nonlinsolve.minIterations;%//由substitutionCompositionUpdate判断K是否收敛
        ...
        function [x, y, K, Z_L, Z_V, L, values] = substitutionCompositionUpdate(model, P, T, z, K, L)
            ...
            % Pure liquid / pure vapor is converged纯液/纯气是收敛的
            ok = L == 1 | L == 0;
            % Compute fugacity ratios计算逸度比
            f_r = bsxfun(@times, sx, f_L./f_V);
            f_r(ok, :) = 1;
            f_r(z == 0) = 1;
            % Update equilibrium constant estimates based on fugacity ratio基于逸度比更新平衡常数
            values = abs(f_r - 1);
            K = max(K.*abs(f_r), 1e-12);
            K(~isfinite(K)) = 1;
        end

        function [stable, x, y] = performPhaseStabilityTest(model, P, T, z, cells, state)
            if nargin < 5
                cells = [];
            end
            if isempty(z)
                ...
                [stable, x, y] = phaseStabilityTest(model, z, P, T, z, z, state, cells);%修改，
            end
        end
        ...
        function state = validateState(model, state)%//验证状态%//初始
        ...
        function [Pr, Tr] = getReducedPT(model, P, T, useCell)%//计算Pr, Tr
            if nargin < 4
                useCell = true;
            end
        ...
        function [A_ij, Bi] = getMixingParameters(model, P, T, acf, useCell)
            ...
            if useCell
                ...
                for i = 1:ncomp
                    for j = i:ncomp
                        A_ij{i, j} = (Ai{i}.*Ai{j}).^(1/2).*(1 - bic(i, j));%//跳转到FastAD？？？
                        A_ij{j, i} = A_ij{i, j};%//混合规则，计算混合物的am和bm,但没有求和
                    end
                end
            else
                Ai = oA.*Pr./Tr.^2;
               ...
               %% %以下为修改
        function [A_ij_V, Bi_V] = getMixingParameters_V(model, P, T, acf, useCell, state)
            if nargin < 5
                useCell = true;
            end
...
P = P + Pcap;
            ...
            [Pr_V, Tr] = model.getReducedPT(P, T, useCell);

            if useCell
                [Ai_V, Bi_V] = deal(cell(1, ncomp));
                [oA, oB] = deal(cell(1, ncomp));
                [oB{:}] = deal(model.omegaB);
            else
                oB = model.omegaB;
            ...
            bic = model.fluid.getBinaryInteraction();
            if useCell
                A_ij_V = cell(ncomp, ncomp);
                for i = 1:ncomp
                    tmp = Pr_V{i}./Tr{i};
                    Ai_V{i} = oA{i}.*tmp./Tr{i};
                    Bi_V{i} = oB{i}.*tmp;
                end
                for i = 1:ncomp
                    for j = i:ncomp
                        A_ij_V{i, j} = (Ai_V{i}.*Ai_V{j}).^(1/2).*(1 - bic(i, j));%//跳转到FastAD？？？
                        A_ij_V{j, i} = A_ij_V{i, j};%//混合规则，计算混合物的am和bm,但没有求和
                    end
                end
            else
                Ai_V = oA.*Pr_V./Tr.^2;
                ...
                A_ij_V = cell(ncomp, 1);
                for j = 1:ncomp
                    A_ij_V{j} = bsxfun(@times, bsxfun(@times, Ai_V, Ai_V(:, j)).^(1/2), 1 - bic(j, :));
                end
            end
        end
  
       %% %以下为修改
        function [A_ij_V, Bi_V] = getMixingParameters_Vtest(model, P, T, acf, useCell, state)
            if nargin < 5
                useCell = true;
            end
...
%                  P = P + pcogi;
        ...
        function [Si_L, Si_V, A_L, A_V, B_L, B_V, Bi, Bi_V] = getMixtureFugacityCoefficients(model, P, T, x, y, acf, state)
            % Calculate intermediate values for fugacity computation计算逸度计算的中间值
            global pcswitch;
            if pcswitch == 1
                [A_ij_V, Bi_V] = model.getMixingParameters_V(P, T, acf, iscell(x), state);%//修改
                [A_ij, Bi] = model.getMixingParameters(P, T, acf, iscell(x));
                [Si_L, A_L, B_L] = model.getPhaseMixCoefficients(x, A_ij, Bi);
                [Si_V, A_V, B_V] = model.getPhaseMixCoefficients(y, A_ij_V, Bi_V);%//修改
            else
                [A_ij_V, Bi_V] = model.getMixingParameters(P, T, acf, iscell(x));
                [A_ij, Bi] = model.getMixingParameters(P, T, acf, iscell(x));
                [Si_L, A_L, B_L] = model.getPhaseMixCoefficients(x, A_ij, Bi);
                [Si_V, A_V, B_V] = model.getPhaseMixCoefficients(y, A_ij_V, Bi_V);
            end %//
        end
        
        function eosdata = getPropertiesFastAD(model, P, T, x, y, z, Z_L, Z_V)%//初始化
            ...
            if iscell(x)
                ...
                x = expandMatrixToCell(x);%//将矩阵扩展为单元格数组。典型用法：转换状态
                y = expandMatrixToCell(y);
            end
            [P, x{:}, y{:}] = initVariablesFastAD(P, x{:}, y{:});%//初始化一些组合代码内部使用的FastAD对象
            [Si_L, Si_V, A_L, A_V, B_L, B_V, Bi, Bi_V] = model.getMixtureFugacityCoefficients(P, T, x, y, model.fluid.acentricFactors);%//修改
            if nargin < 7
                Z_L = model.computeCompressibilityZ(P, x, A_L, B_L, Si_L, Bi, true);
                Z_V = model.computeCompressibilityZ_V(P, y, A_V, B_V, Si_V, Bi_V, false);%//修改
            ...
            f_V = model.computeFugacity_V(P, y, Z_V, A_V, B_V, Si_V, Bi_V);%//修改
            eosdata = struct();
            eosdata.Z_L = Z_L;
            eosdata.Z_V = Z_V;
            eosdata.f_L = f_L;
        ...
        %% %以下部分为修改
        function [Z_L, Z_V, f_L, f_V] = getProperties_update(model, P, T, x, y, z, sO, sG, state, varargin)
            if nargin < 9
                state = [];
            end
            [Si_L, Si_V, A_L, A_V, B_L, B_V, Bi, Bi_V] = model.getMixtureFugacityCoefficients(P, T, x, y, model.fluid.acentricFactors, state);%//修改
            Z_L = model.computeCompressibilityZ(P, x, A_L, B_L, Si_L, Bi, true);%//计算压缩系数ZL
            Z_V = model.computeCompressibilityZ_V(P, y, A_V, B_V, Si_V, Bi_V, false, state);%//修改           
            if iscell(x)
            ...
            f_L = model.computeFugacity(P, x, Z_L, A_L, B_L, Si_L, Bi);
            f_V = model.computeFugacity_V(P, y, Z_V, A_V, B_V, Si_V, Bi_V, state);
        end
        %%
        ...
        %% %//以下为修改，复制计算Fugacity_V
         function [f, phi] = computeFugacity_V(model, p, x, Z, A, B, Si, Bi, state)
            % Compute fugacity based on EOS coefficients
            [m1, m2] = model.getEOSCoefficients();
            global pcswitch fluidint pcapillary;
             if pcswitch == 1
...
p = p + Pcap;
        ...
        function [x, y, LL, packed] = getPhaseFractionAsADI(model, state, pP, TP, z0)
            ...
            % mole fraction).计算通过求解平衡方程得到的值的导数（每相中的摩尔分数，液体摩尔分数）。
        ...
        function L = solveRachfordRice(model, L, K, z) %#ok
            L = solveRachfordRiceVLE(L, K, z);%//气液分化率
        end
        ...
        function y = computeVapor(model, L, K, z)
            ...
            if iscell(z)
                ...
                y = bsxfun(@rdivide, y, sum(y, 2));%//存在问题
        ...
        function frac = getMassFraction(model, molfraction)
            % Convert molar fraction to mass fraction%//将摩尔分数转换为质量分数
        ...
        function [state, report] = updateAfterConvergence(model, state0, state, dt, drivingForces)
            ...
            % Set derivatives
            state.eos.packed = model.getPropertiesFastAD(state.pressure, state.T, state.x, state.y, state);%//修改
        end
