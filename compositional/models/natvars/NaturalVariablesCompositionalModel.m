classdef NaturalVariablesCompositionalModel < ThreePhaseCompositionalModel
    ...
    methods
        ...
        function [ds, deltax, deltay, dL, dx, vars] = getHyperbolicUpdates(model, problem, dx, vars, state)
            ...
            [deltax, deltay, dx, vars] = model.getPhaseCompositionIncrements(dx, vars, state);%//获得饱和度增量和组分增量
        ...
        function [state, report] = updateState(model, state, problem, dx, drivingForces)
            ...
            % Get saturation and composition updates%//获得饱和度和组分更新
        ...
        function state = flashPhases(model, state, state0, s_uncap, xyUpdated, iteration)
            ...
            if 0
                ...
                [stable(act), x(act, :), y(act, :)] =...
                    model.EOSModel.performPhaseStabilityTest(p(act, :), T(act, :), z(act, :), act, state);
            end
...
[Pcap] = computePcapfinal(model.EOSModel.fluid, rho_L,rho_V,state.x,state.y);
p_V = p + Pcap;
             else
                 p_V = p;
             end %//添加毛管力
            [Z_L, Z_V] = eos.getProperties_update(p, temp, x, y, z, sO, sG, state);%//修改，有问题
            if isa(model, 'PressureNaturalVariablesModel')
                ...
                rhoG = model.PropertyModel.computeMolarDensity(p_V, y, Z_V, temp, false);%//计算密度
        ...
        function [xM,  yM,  rhoO,  rhoG,  muO,  muG, f_L, f_V,...
            ...
            [Z_L, Z_V, f_L, f_V] = eos.getProperties(p, temp, x, y, z, sO, sG, state, cellJacMap);%//里面修改

            [xM, rhoO, muO] = model.getFlowPropsNatural(p, x, Z_L, temp, true);
            [yM, rhoG, muG] = model.getFlowPropsNatural_V(p, y, Z_V, temp, false, state);%//里面修改
            ...
            [yM0, rhoG0] = model.getFlowPropsNatural_V(p0, y0, state0.Z_V, temp0, false, state0);%//里面修改
            %------------------------------------添加临界性质偏移-------------------------------------------%
%             [Pcap] = computePcapadifinal(eos.fluid, radius,rhoO,rhoG,x,y);
%             sample = p; 
%             Pcapadi = model.AutoDiffBackend.convertToAD(Pcap, sample);            
%             state.Pcap = Pcap;
%             state.Pcapadi = Pcapadi;
%             [Z_L, Z_V, f_L, f_V] = eos.getProperties(p, temp, x, y, z, sO, sG, state, cellJacMap);%//里面修改
% 
%             [xM, rhoO, muO] = model.getFlowPropsNatural(p, x, Z_L, temp, true);
%             [yM, rhoG, muG] = model.getFlowPropsNatural_V(p, y, Z_V, temp, false, state);%//里面修改
...
%             y0 = expandMatrixToCell(y0);
%             
%             [Pcap0] = computePcapfinal(eos.fluid, radius,rhoO0,rhoG0,x0,y0);
% 
%             [xM0, rhoO0] = model.getFlowPropsNatural(p0, x0, state0.Z_L, temp0, true);
%             [yM0, rhoG0] = model.getFlowPropsNatural_V(p0, y0, state0.Z_V, temp0, false, state0);%//里面修改
        end
        ...
        function [xM, rho_V, mu] = getFlowPropsNatural_V(model, p, x, Z, T, isLiquid, state)%//修改，添加
            xM = model.EOSModel.getMassFraction(x);
...
p = p + Pcap;
            ...
            rho_V = model.PropertyModel.computeDensity(p, x, Z, T, isLiquid);
            if nargout > 2
                mu = model.PropertyModel.computeViscosity(p, x, Z, T, isLiquid);
            end
        end
