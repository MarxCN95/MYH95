...
Pcap = Pcap*0;
%Pcap = Pcap(cells, :);
p_V = p + Pcap;%修改， IFT计算毛管力
    [y, S_V, isTrivialV] = checkStability_V(eos, z, y, p_V, T, true, active, opt, state);%修改
    else
         [y, S_V, isTrivialV] = checkStability(eos, z, y, p, T, true, active, opt);%修改   
    end
    V_stable = S_V <= 1.1 + S_tol | isTrivialV;%误差放大到0.01%
...
%     end                   % 2021年11月4日22:25:06  修改
%     xy_loc(1,7)==1.892192800794860e-04后出现
    ...
    if numelnan > 1
       warning('numelnan have NaN'); 
    end     % 2021年11月4日22:25:06  修改 个别组分 xy_loc会出现无限大的值，导致R出现NaN空值
        K(active, :) = K(active, :).*R;
        R_norm = sum((R-1).^2, 2);
